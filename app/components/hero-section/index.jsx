        ...
        <div className="flex flex-col items-start justify-center rounded-lg p-3 lg:py-5 lg:px-12 bg-primary-bg h-full">
          ...
          <div className="w-full flex justify-center items-center gap-5">
            {userData.socials.map((social, index) => (
              <Link
                ...
                href={social.title}
                target="_blank"
                className="transition-all duration-300 hover:scale-125 text-teal-500"
              >
                {social.icon}
              </Link>
            ))}
          ...
          <div className="w-full justify-center flex items-center gap-3 mt-6">
            <Link
              ...
              className="rounded-full transition-all duration-300 bg-gradient-to-r to-pink-500 from-violet-600 p-[1px] hover:from-pink-500 hover:to-violet-600"
            >
              <button className="flex items-center gap-1 px-3 text-xs md:px-4 py-2 md:py-2.5 bg-[#0d1224] rounded-full border-none text-center font-medium uppercase tracking-wider hover:tracking-widest text-[#ffff] transition-all duration-300 md:font-semibold hover:gap-3">
          ...
          <div className="overflow-hidden border-t-[2px] border-indigo-900 px-4 lg:px-8 py-4 lg:py-8">
            <code className="font-mono text-xs md:text-sm lg:text-base">
              ...
              <div className="ml-4 lg:ml-8 mr-2">
                ...
                <span className="text-gray-400">{`['`}</span>
                {userData.frameworks.map((skill, i) => (
                  <>
              ...
              <div className="ml-4 lg:ml-8 mr-2">
                ...
                <span className="text-gray-400">{`['`}</span>
                {userData.libraries.map((library, i) => (
                  <>
                    <span className="text-cyan-400">{library}</span>
                    {i !== userData.libraries.length - 1 && (
                      <span className="text-gray-400">{"', '"}</span>
                    )}
                  </>
                ))}
